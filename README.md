...
This SDK facilitates interactions with the contracts in [Universal Router](https://gitlab.snapresearch.io/swapnity/cytoswap-universal-router)
...
### Trading ERC20s on Cytoswap
warning: `swapERC20CallParameters()` to be deprecated in favor of `swapCallParameters()`
```typescript
import { TradeType } from '@cytoswap/sdk-core'
import { Trade as V2TradeSDK } from '@cytoswap/v2-sdk'
import { Trade as V3TradeSDK } from '@cytoswap/v3-sdk'
import { MixedRouteTrade, MixedRouteSDK, Trade as RouterTrade } from '@cytoswap/router-sdk'

const options = { slippageTolerance, recipient }
const routerTrade = new CytoswapTrade(
...
### Using Cytoswap for ERC20 NFT Trades
Send ETH to the router by trading an ERC20 for ETH with a Cytoswap Trade and encoding the swap recipient as `ROUTER_AS_RECIPIENT` in the trade. Then subsequently list the NFT trades to use the ETH output to buy NFTs. Trades happen in the order they are listed.
...
const cytoswapTrade = new CytoswapTrade(
...
// Use the raw calldata and value returned to call into Universal Swap Router contracts
const { calldata, value } = SwapRouter.swapCallParameters([cytoswapTrade, seaportTrades, looksRareTrades])
```

### Using WETH for NFT Trades
The current router purchases all NFTs with ETH, but you can send WETH to the router to be unwrapped for ETH right before the NFT commands. Similar to ERC20 Cytoswap Trades for NFTs, supplemental ETH will be sent in the transaction if the WETH amount will not cover the NFT buys. You can also use ERC20s and WETH to cover the transaction by including both commands before the NFT purchase.
